...
module.exports = () => {
  return async ctx => {
    ...
    if (!ethers.utils.isAddress(rebateId)) {
      const getReserveDetailsById = helpers.getStorageFunction(network, 'getReserveDetailsById');
    ...
    const getRebateAmount = helpers.getFeeHandlerFunction(network, 'rebatePerWallet');
    result = await getRebateAmount(rebateId);

    replyWithMarkdown(
      `Rebate Amt: \`${helpers.getReadableWei(result)} ETH\``,
      inReplyTo(message.message_id)
    );
  };
};
